...
def patched_unet_forward(self, x, timesteps=None, context=None, y=None, control=None, transformer_options={}, **kwargs):
    ...
    if self.current_step.dim() == 0:
        patch_settings[os.getpid()].global_diffusion_progress = float(self.current_step.detach().cpu().item())
    else:
        patch_settings[os.getpid()].global_diffusion_progress = float(self.current_step[0].detach().cpu().item())
    ...
    if self.num_classes is not None:
        assert y.shape[0] == x.shape[0]
        emb = emb + self.label_emb(y.to(emb.device).to(emb.dtype))
